...
\title{Caduceus-Flux Microservices Manual (IEEE Conference Format)}
...
\begin{abstract}
This document provides a textbook-style, evidence-based manual for the Caduceus-Flux microservices platform. All claims are derived directly from the codebase and configuration files, with explicit evidence references (file path and line numbers). The goal is to help operators and developers understand how each microservice works, how to use the APIs, and how the system is composed in practice.
\end{abstract}

\section{Scope and Evidence Rules}
This manual only reports what is explicitly present in code and configuration. Every important statement includes evidence formatted as \texttt{path}, lines X--Y. If an item is not shown in code/config, it is not asserted as present.

\section{Reader's Guide}
\begin{itemize}
\item \textbf{Microservice Catalog}: a compact overview (ports, containers, roles).
\item \textbf{Core Emulation and Topology Services}: the main lifecycle and topology control path.
\item \textbf{Snapshot, Export/Import, Generation}: reproducibility and topology automation.
\item \textbf{Programmable Data Plane (P4)}: compilation and BMv2 runtime.
\item \textbf{Observability and Analytics}: metrics, streaming, and decisions.
\item \textbf{Control Plane and Gateways}: MCP routing and config orchestration.
\item \textbf{AI/LLM and MCP Tooling}: LLM gateway, tool scoping, proxying.
\item \textbf{MANO/NFV Integration}: MANO core, OSM connector, VIM emulation.
\item \textbf{Supporting Infrastructure}: databases, brokers, analytics stack.
\end{itemize}

\section{Microservice Catalog}
\begin{table*}[t]
\centering
\caption{Microservice Catalog}
\renewcommand{\arraystretch}{1.1}
\begin{tabularx}{\textwidth}{p{3.3cm}p{2.1cm}p{4.2cm}Y}
\toprule
...
\item \texttt{POST /api/topologies}, \texttt{GET /api/topologies}: topology CRUD. \texttt{backend/services/topology/topology\_app/app.py}, lines 398--405 and 619--620.
...
\textbf{Purpose.} Unified runtime for Mininet, Mininet-WiFi, and Containernet with BMv2 (P4) support, exposing gRPC control. \texttt{emulation-container/Dockerfile}, lines 1--66.
...
\textbf{Purpose.} Multi-format topology export (Mininet, Mininet-WiFi, Containernet, GraphML, JSON, YAML). \texttt{backend/services/export\_import/export\_import\_app/app.py}, lines 980--1003.
...
These components are not microservices but are required for the platform to run.

\begin{itemize}
\item \textbf{PostgreSQL, MongoDB, Redis}: data and state stores. \texttt{docker-compose.yml}, lines 1--121.
\item \textbf{InfluxDB, Prometheus, Grafana}: telemetry storage and dashboards. \texttt{docker-compose.yml}, lines 122--227.
\item \textbf{RabbitMQ, Consul}: messaging and service discovery. \texttt{docker-compose.yml}, lines 147--183.
...
\item \textbf{Hive, Hue}: SQL on Hadoop and UI. \texttt{docker-compose.yml}, lines 1026--1145.
